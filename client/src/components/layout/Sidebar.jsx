...
export default function Sidebar({ links = [], onLoginClick, onRegisterClick }) {
  ...
  // Logout handler with JWT token invalidation
  const handleLogout = async () => {
    try {
      ...
      ['token', 'role', 'userId', 'userName', 'user'].forEach(key => 
    ...
    <>
      ...
      <div className={`space-y-4 transform transition-all duration-700 ${
        ...
        {isAuthenticated ? (
          <>
            <div 
              className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-4 border border-gray-700/50 cursor-pointer hover:bg-gray-700/50 transition-all duration-300 hover:border-amber-500/50 group"
              onClick={() => {
            ...
            >
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 bg-gradient-to-br from-amber-400 to-rose-500 rounded-full flex items-center justify-center text-white font-bold group-hover:scale-110 transition-transform duration-300">
                  {localStorage.getItem('userName')?.[0] || 'U'}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate group-hover:text-amber-400 transition-colors duration-300">
